...
(function () {
    ...
    Markdown.Editor = function (markdownConverter, idPostfix, options) {
        ...
        this.run = function () {
            ...
            uiManager = new UIManager(this.editor, idPostfix, panels, undoManager, previewManager, commandManager, options.helpButton, getString);
            uiManager.setUndoRedoButtonStates();
    ...
    function UIManager(editor, postfix, panels, undoManager, previewManager, commandManager, helpOptions, getString) {
        ...
        function makeRightButtonRow() {
            var buttonBar = panels.buttonBar;
            var buttonRow = document.createElement("ul");
            buttonRow.id = "wmd-button-right-row" + postfix;
            buttonRow.className = 'wmd-button-row';
            buttonRow = buttonBar.appendChild(buttonRow);

            var makeButton = function (id, title, XShift, textOp) {
                var button = document.createElement("li");
                button.className = "wmd-button pull-right " + id;
            ...
            var makeSpacer = function (num) {
                ...
                spacer.className = "wmd-spacer wmd-right-spacer" + num + " pull-right";
                spacer.id = "wmd-right-spacer" + num + postfix;
                buttonRow.appendChild(spacer);
            }

            buttons.previewMode = makeButton("wmd-preview-mode-button", getString("previewMode"), "-360px");
            buttons.previewMode.execute = bindCommand(function () {
                return this.doMode(editor, 'preview-mode');
            });
            buttons.liveMode = makeButton("wmd-live-mode-button", getString("liveMode"), "-340px");
            buttons.liveMode.execute = bindCommand(function () {
                return this.doMode(editor, 'live-mode');
            });
            buttons.editMode = makeButton("wmd-edit-mode-button", getString("editMode"), "-320px");
            buttons.editMode.execute = bindCommand(function () {
                return this.doMode(editor, 'edit-mode');
            });
            makeSpacer(1);
            buttons.fullscreen = makeButton("wmd-fullscreen-button", getString("fullscreen"), "-240px");
            buttons.fullscreen.execute = bindCommand(function () {
                return this.doFullScreen(editor, buttons);
            });
        }
    ...
    commandProto.doFullScreen = function (editor, buttons) {
        var cls = 'fullscreen';
        if (!hasClass(editor, cls)) {
            addClass(editor, cls);
            addClass(document.body, 'wmd-editor-fullscreen');
            buttons.fullscreen.XShift = "-260px";
        } else {
            removeClass(editor, cls);
            removeClass(document.body, 'wmd-editor-fullscreen');
            buttons.fullscreen.XShift = "-240px";
        }
    }

    commandProto.doMode = function (editor, mode) {
        removeClass(editor, 'preview-mode');
        removeClass(editor, 'live-mode');
        removeClass(editor, 'edit-mode');
...
(function () {
    ...
    var mathJax = Markdown.MathJax = {};
    var preview = null;

    mathJax.init = function (editor, postfix) {
