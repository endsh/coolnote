...
from .html import clean_html, doc2html, tags
from .urls import url2format
...
class Marker(object):
	...
	def format(self, tag, node=None, attrs=None, center=False):
		...
		attrs = ['%s=%s' % (x, quoteattr(y)) for x, y in attrs.iteritems() if y is not None]
	...
	def start(self, node=None, end=False):
		if node is not None:
			if node.tag in BLOCKS and node.tag not in SKIPS or node.tag in 'br':
...
class Cleaner(object):
	...
	def __init__(self, input, **options):
		...
		self.doc = clean_html(input, todoc=True)
	...
	def clean_bads(self):
		...
		# 				or 'themes' in src \
		# 				or (url2format(src) or '').lower() not in jpgs \
	...
	def clean_none(self, node):
		if re.search('display: *none|opacity: *0', node.get('style', '')):
			self.drop(node)
			return

		for child in node.getchildren():
			self.clean_none(child)
	...
	def clean_nodes(self):
		...
		for child in children:
			self.clean_none(child)
			if re.search('display: *none|opacity: *0', child.get('style', '')):
				self.drop(child)
				continue
			...
			if BADS.search(text) and node['text_len'] < 300:
				self.drop(child)
				node['drop'] = True
